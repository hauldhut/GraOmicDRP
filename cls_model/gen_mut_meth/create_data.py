...
def save_mix_drug_cell_matrix():#(max_samples=10):
    ...
    print(len(cell_dict_meth))

    print(len(cell_dict_ge))
    ...
    for data in temp_data:
        drug, cell, ic50 = data
        if drug in drug_dict and cell in cell_dict_mut and cell in cell_dict_meth and cell in cell_dict_ge:
            if test_index[(int(cell), drug)] == 1:
                ...
                xc_meth_test_temp.append(cell_dict_meth[cell])
    ...
    train_data = TestbedDataset(root='data', dataset=dataset+'_train_mix', xd=xd_train, xt_mut=xc_mut_train, xt_meth=xc_meth_train, xt_ge=xc_ge_train, y=y_train, smile_graph=smile_graph)
    val_data = TestbedDataset(root='data', dataset=dataset+'_val_mix', xd=xd_val, xt_mut=xc_mut_val, xt_meth=xc_meth_val, xt_ge=xc_ge_val, y=y_val, smile_graph=smile_graph)
    test_data = TestbedDataset(root='data', dataset=dataset+'_test_mix', xd=xd_test, xt_mut=xc_mut_test, xt_meth=xc_meth_test, xt_ge=xc_ge_test, y=y_test, smile_graph=smile_graph)
