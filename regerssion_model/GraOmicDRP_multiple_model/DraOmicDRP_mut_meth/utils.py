...
class TestbedDataset(InMemoryDataset):
    def __init__(self, root='/tmp', dataset='davis', 
                 xd=None, xt_mut=None, xt_meth=None, y=None, transform=None,
        ...
        if os.path.isfile(self.processed_paths[0]):
            ...
            self.process(xd, xt_mut, xt_meth, y, smile_graph)
            self.data, self.slices = torch.load(self.processed_paths[0])
    ...
    def process(self, xd, xt_mut, xt_meth, y, smile_graph):
        assert (len(xd) == len(xt_mut) and len(xt_mut) == len(y)) and len(y) == len(xt_meth), "The four lists must be the same length!"
        data_list = []
        data_len = len(xd)
